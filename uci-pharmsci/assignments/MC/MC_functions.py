...
"""
    ...
    Dists = [0., dx]
    PEs = [mclib.calcenergy(Pos + NormDir * x, M, L, Cut) for x in Dists]
    ...
    while PEs[1] > PEs[0]:
        ...
        dx = dx * 0.5
        Dists[1] = dx
        PEs[1] = mclib.calcenergy(Pos + NormDir * dx, M, L, Cut)

    #find a third point
    Dists = Dists + [2. * dx]
    PEs = PEs + [mclib.calcenergy(Pos + NormDir * 2. * dx, M, L, Cut)]
    ...
    while PEs[2] < PEs[1]:
        ...
        PEs = PEs + [mclib.calcenergy(Pos + NormDir * Dists[-1], M, L, Cut)]
    ...
    while True:
        ...
        PE3 = mclib.calcenergy(Pos + NormDir * d3, M, L, Cut)
...
"""
    PE, Forces = mclib.calcenergyforces(Pos, M, L, Cut, np.zeros_like(Pos))
    Dir = Forces
    OldPE = 1.e300
