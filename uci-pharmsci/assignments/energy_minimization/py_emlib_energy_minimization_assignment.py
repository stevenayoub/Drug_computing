...
def minimize_clusters(num_iterations, N_min, N_max, density, dx, EFracTolCG, EFracTolLS, nprocs):
    """
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Parse system minimization parameters.")

    parser.add_argument(
        '--num_iterations',
        type=int,
        required=True,
        help='The number of iterations to minimize the system.'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help='Minimum number of particles. Default: 2'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help='Maximum number of particles. Default: 25'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help='Density of the system. Default: 0.001'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0e-10,
        help='Fractional change in energy for CG. Default: 1.0e-10'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0e-8,
        help='Fractional change in energy for LS. Default: 1.0e-8'
    )
    parser.add_argument(
        '--Nprocs',
        type=float,
        default=None,
        help='Number of cores to be used. Default: All CPUS '
    )
...
if __name__ == "__main__":
    ...
    if args.Nprocs is None:
        args.Nprocs = num_cpus
        print(f'Using all {args.Nprocs} Cores')
    else:
        print(f'Using only: {args.Nprocs} Cores out of {num_cpus}')
    minimize_clusters(num_iterations=args.num_iterations, N_min=args.N_min, N_max=args.N_max,
                      density=args.density, dx=args.dx, EFracTolCG=args.EFracTolCG,
                      EFracTolLS=args.EFracTolLS, nprocs=args.Nprocs)
    end_time = time.time()
